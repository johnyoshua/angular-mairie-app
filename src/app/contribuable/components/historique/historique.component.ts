...
export class HistoriqueComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name','adresse','num_paiement','montant','impression'];
...
loadData(){
    this.isLoading = true;
    this.service.getHistorique(this.route.snapshot.paramMap.get('id'),this.mainForm.getRawValue()).pipe(
      tap((data:any)=>{
        if(data){
          this.dataSource.data = data.liste;
          this.contribuable=data.one;
          setTimeout(() => {
  ...
  RapportPDF(hiatoriques: any, dataC:any) {
    ...
    if(this.date_fin==undefined){
      ...
      const docDefinition = {
          ...
          content: [
            {
              text: 'HISTORIQUE DU CONTRIBUABLE '+dataC.denomination+', MATRICULE: '+dataC.matricule+' '+ dateDebut+' '+dateFin,
              fontSize: 14,
              bold: true,
              alignment: 'center',
              margin:[ 0, 0, 0, 12 ],
...

            {
              table: {
                ...
                body: [
                  ['DATE','TAXES', 'N° RECU', 'MONTANT'],
                  ...hiatoriques.map((p: {date_paiement: string; libelle:string; num_paiement:number; montant: number })=> ([datepipe.transform(p.date_paiement, 'dd/MM/YYYY'), p.libelle, p.num_paiement, this.number(p.montant)])),
                    [{ text: 'TOTAL', colSpan: 3 }, {},{}, this.number(this.getTotalMontant())],
                ]
              }
            },
